...
namespace TH09 {
    ...
    protected:
        ...
        virtual void OnLocaleChange() override {
            this->SetTitle(Gui::LocaleGetStr(TH09_TOOLS_TITLE));
        }
        ...
        virtual void OnContentUpdate() override {
            ...
            ImGui::TextUnformatted(Gui::LocaleGetStr(TH_MUTEKI));
            ImGui::Checkbox(Gui::LocaleGetStr(TH09_P1), &invinc_p1);
            ImGui::SameLine();
            ImGui::Checkbox(Gui::LocaleGetStr(TH09_P2), &invinc_p2);
            ImGui::PopID();

            ImGui::PushID(TH_HEALTH);
            ImGui::TextUnformatted(Gui::LocaleGetStr(TH_HEALTH));
            ImGui::PushID(TH09_P1);
            ImGui::SliderInt("", (int*)(pl1 + 0xa8), 1, 10, Gui::LocaleGetStr(TH09_P1_FORMAT_INT));
            ImGui::SameLine();
            ImGui::Checkbox(Gui::LocaleGetStr(TH09_LOCK), &infhealth_p1);
            ImGui::PopID();
            ImGui::PushID(TH09_P2);
            ImGui::SliderInt("", (int*)(pl2 + 0xa8), 1, 10, Gui::LocaleGetStr(TH09_P2_FORMAT_INT));
            ImGui::SameLine();
            ImGui::Checkbox(Gui::LocaleGetStr(TH09_LOCK), &infhealth_p2);
            ImGui::PopID();
            ImGui::PopID();

            ImGui::PushID(TH09_INSTANT_DEATH);
            ImGui::TextUnformatted(Gui::LocaleGetStr(TH09_INSTANT_DEATH));
            ImGui::PushID(TH09_P1);
            if (ImGui::Button(Gui::LocaleGetStr(TH09_P1))) {
                *(uint32_t*)(pl1 + 0xa8) = 0;
            ...
            ImGui::SameLine();
            ImGui::PushID(TH09_P2);
            if (ImGui::Button(Gui::LocaleGetStr(TH09_P2))) {
                *(uint32_t*)(pl2 + 0xa8) = 0;
            ...
            auto chargegauge = [&](uint32_t pl, bool* lock_check) {
                ...
                const char* _p;
                if (*(uint32_t*)(pl + 8)) {
                    _p = Gui::LocaleGetStr(TH09_P2_FORMAT_FLOAT);
                    ImGui::PushID(TH09_P2);
                } else {
                    _p = Gui::LocaleGetStr(TH09_P1_FORMAT_FLOAT);
                    ImGui::PushID(TH09_P1);
                }
                ...
                if (ImGui::IsItemHovered()) {
                    ...
                    ImGui::TextUnformatted(Gui::LocaleGetStr(TH09_LOCK));
            ...
            ImGui::TextUnformatted(Gui::LocaleGetStr(TH09_CHARGE_GAUGE));
            ...
            if (*(uint32_t*)0x4a7db8 || *(uint32_t*)0x4a7df0) {
                ...
                ImGui::TextUnformatted(Gui::LocaleGetStr(TH09_CPU_CHARGE));
                if (*(uint32_t*)0x4a7db8) {
                    ImGui::PushID(TH09_P1);
                    cpu_charge_p1 = (int)*(float*)(pl1 + 0x7c) / 100;
                    if (ImGui::SliderInt(Gui::LocaleGetStr(TH09_P1), &cpu_charge_p1, 1, 4)) {
                        *(float*)(pl1 + 0x7c) = static_cast<float>(cpu_charge_p1) * 100;
                    }
                    ImGui::SameLine();
                    ImGui::Checkbox(Gui::LocaleGetStr(TH09_LOCK), &cpu_lock_attack_p1);
                    ImGui::PopID();
                }
                if (*(uint32_t*)0x4a7df0) {
                    ImGui::PushID(TH09_P2);
                    cpu_charge_p2 = (int)*(float*)(pl2 + 0x7c) / 100;
                    if (ImGui::SliderInt(Gui::LocaleGetStr(TH09_P2), &cpu_charge_p2, 1, 4)) {
                        *(float*)(pl2 + 0x7c) = static_cast<float>(cpu_charge_p2) * 100;
                    }
                    ImGui::SameLine();
                    ImGui::Checkbox(Gui::LocaleGetStr(TH09_LOCK), &cpu_lock_attack_p2);
                    ImGui::PopID();
                }
                ImGui::PopID();
            }
            ...
            ImGui::TextUnformatted(Gui::LocaleGetStr(TH09_RANK));
            ImGui::SliderInt(Gui::LocaleGetStr(TH09_RANK), (int*)0x4a7e44, 1, *(uint32_t*)0x4a7e58);
            ImGui::InputInt(Gui::LocaleGetStr(TH09_RANK_MAX), (int*)0x4a7e58, 1, 5);
            ImGui::InputInt(Gui::LocaleGetStr(TH09_RANK_INC), (int*)0x4a7e54, 1, 1);
            if (ImGui::Checkbox(Gui::LocaleGetStr(TH09_LOCK), &ranklock)) {
                th09_ranklock.Toggle(ranklock);
            }
            ImGui::PopID();

            ImGui::PushID(TH09_LV_RANK);
            ImGui::TextUnformatted(Gui::LocaleGetStr(TH09_LV_RANK));
            ImGui::SliderInt(Gui::LocaleGetStr(TH09_P1), (int*)(pl1 + 0xa0), 1, 16);
            ImGui::SliderInt(Gui::LocaleGetStr(TH09_P2), (int*)(pl2 + 0xa0), 1, 16);
            ImGui::PopID();

            ImGui::PushID(TH09_BOSS_RANK);
            ImGui::TextUnformatted(Gui::LocaleGetStr(TH09_BOSS_RANK));
            ImGui::SliderInt(Gui::LocaleGetStr(TH09_P1), (int*)(pl1 + 0xa4), 1, 16);
            ImGui::SliderInt(Gui::LocaleGetStr(TH09_P2), (int*)(pl2 + 0xa4), 1, 16);
            ImGui::PopID();

            ImGui::PushID(TH09_MISC);
            ImGui::TextUnformatted(Gui::LocaleGetStr(TH09_MISC));
            ImGui::SliderInt(Gui::LocaleGetStr(TH09_AI_POWER), (int*)0x4a7df4, 1, 60);
            ImGui::InputInt(Gui::LocaleGetStr(TH09_LILY_TIMER), (int*)0x4a7e5c, 1, 3);
            ImGui::PopID();

            if (BeginOptGroup<TH09_OCCURENCE>()) {
                ImGui::Checkbox(Gui::LocaleGetStr(TH09_O_LILY), &o_lily);
                ImGui::Checkbox(Gui::LocaleGetStr(TH09_O_FAIRY), &o_fairy);
                ImGui::Checkbox(Gui::LocaleGetStr(TH09_O_PELLETS_RANDOM), &o_pellets_random);
                ImGui::Checkbox(Gui::LocaleGetStr(TH09_O_PELLETS_RIVAL), &o_pellets_rival);
                ImGui::Checkbox(Gui::LocaleGetStr(TH09_O_LARGE_BULLETS), &o_large_bullets);
                ImGui::Checkbox(Gui::LocaleGetStr(TH09_O_SPIRITS_RANDOM), &o_spirits_random);
                ImGui::Checkbox(Gui::LocaleGetStr(TH09_O_SPIRITS_RIVAL), &o_spirits_rival);
                ImGui::Checkbox(Gui::LocaleGetStr(TH09_O_EX), &o_ex);
                ImGui::Checkbox(Gui::LocaleGetStr(TH09_O_LV2), &o_lv2);
                ImGui::Checkbox(Gui::LocaleGetStr(TH09_O_LV3), &o_lv3);
                ImGui::Checkbox(Gui::LocaleGetStr(TH09_O_BOSS), &o_boss);
                EndOptGroup();
            }

            auto show_stats = [](uint32_t b) {
                ImGui::TextUnformatted(Gui::LocaleGetStr(TH_BULLETS));
                ImGui::Indent();
                ImGui::Text(Gui::LocaleGetStr(TH09_STAT_B_FAIRY), *(uint32_t*)(b + 0x25e164));
                ImGui::Text(Gui::LocaleGetStr(TH09_STAT_B_RIVAL), *(uint32_t*)(b + 0x25e168));
                ImGui::Text(Gui::LocaleGetStr(TH09_STAT_B_TOTAL), *(uint32_t*)(b + 0x25e16c));
                ImGui::Unindent();
            };

            ImGui::TextUnformatted(Gui::LocaleGetStr(TH09_STATS));
