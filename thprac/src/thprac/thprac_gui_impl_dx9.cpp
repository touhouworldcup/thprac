...
namespace Gui {
    ...
    static bool ImplDX9CreateFontsTexture()
    {
        ...
        g_FontTexture = NULL;
        if (g_pd3dDevice->lpVtbl->CreateTexture(g_pd3dDevice, width, height, 1, D3DUSAGE_DYNAMIC, D3DFMT_A8R8G8B8, D3DPOOL_DEFAULT, &g_FontTexture, NULL) < 0)
            return false;
    ...
    static void ImplDX9SetupRenderState(ImDrawData* draw_data)
    {
        ...
        g_pd3dDevice->lpVtbl->SetRenderState(g_pd3dDevice, D3DRS_SCISSORTESTENABLE, true);
        ...
        // Being agnostic of whether <d3dx9.h> or <DirectXMath.h> can be used, we aren't relying on D3DXMatrixIdentity()/D3DXMatrixOrthoOffCenterLH() or DirectX::XMMatrixIdentity()/DirectX::XMMatrixOrthographicOffCenterLH()
        {
            ...
            g_pd3dDevice->lpVtbl->SetTransform(g_pd3dDevice, D3DTS_PROJECTION, &mat_projection);
        }
    }
    ...
    static __forceinline void ImplDX9StateRestore()
    {
        ...
        g_pOrigStateBlock->lpVtbl->Apply(g_pOrigStateBlock);
        g_pOrigStateBlock->lpVtbl->Release(g_pOrigStateBlock);
    ...
    void ImplDX9AdjustDispSize()
    {
        ...
        g_pd3dDevice->lpVtbl->GetBackBuffer(g_pd3dDevice, 0, 0, D3DBACKBUFFER_TYPE_MONO, &backBuffer);
    ...
    void ImplDX9RenderDrawData(ImDrawData* draw_data)
    {
        ...
        if (!g_pVB || g_VertexBufferSize < draw_data->TotalVtxCount) {
            ...
            if (g_pd3dDevice->lpVtbl->CreateVertexBuffer(g_pd3dDevice, g_VertexBufferSize * sizeof(CUSTOMVERTEX), D3DUSAGE_DYNAMIC | D3DUSAGE_WRITEONLY, D3DFVF_CUSTOMVERTEX, D3DPOOL_DEFAULT, &g_pVB, NULL) < 0)
                return;
        }
        if (!g_pIB || g_IndexBufferSize < draw_data->TotalIdxCount) {
            if (g_pIB) {
                g_pIB->lpVtbl->Release(g_pIB);
                g_pIB = NULL;
            }
            g_IndexBufferSize = draw_data->TotalIdxCount + 10000;
            if (g_pd3dDevice->lpVtbl->CreateIndexBuffer(g_pd3dDevice, g_IndexBufferSize * sizeof(ImDrawIdx), D3DUSAGE_DYNAMIC | D3DUSAGE_WRITEONLY, sizeof(ImDrawIdx) == 2 ? D3DFMT_INDEX16 : D3DFMT_INDEX32, D3DPOOL_DEFAULT, &g_pIB, NULL) < 0)
                return;
        }
        ...
        ImDrawIdx* idx_dst;
        if (g_pVB->lpVtbl->Lock(g_pVB, 0, (UINT)(draw_data->TotalVtxCount * sizeof(CUSTOMVERTEX)), (void**)&vtx_dst, D3DLOCK_DISCARD) < 0)
            return;
        if (g_pIB->lpVtbl->Lock(g_pIB, 0, (UINT)(draw_data->TotalIdxCount * sizeof(ImDrawIdx)), (void**)&idx_dst, D3DLOCK_DISCARD) < 0)
            return;
        ...
        g_pd3dDevice->lpVtbl->SetIndices(g_pd3dDevice, g_pIB);
        ...
        for (int n = 0; n < draw_data->CmdListsCount; n++) {
            ...
            for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++) {
                ...
                if (pcmd->UserCallback != NULL) {
                    ...
                    const LPDIRECT3DBASETEXTURE9 texture = (LPDIRECT3DBASETEXTURE9)pcmd->TextureId;
                    g_pd3dDevice->lpVtbl->SetTexture(g_pd3dDevice, 0, texture);
                    g_pd3dDevice->lpVtbl->SetScissorRect(g_pd3dDevice, &r);
                    g_pd3dDevice->lpVtbl->DrawIndexedPrimitive(g_pd3dDevice, D3DPT_TRIANGLELIST, vtx_offset, 0, (UINT)cmd_list->VtxBuffer.Size, idx_offset, pcmd->ElemCount / 3);
                }
    ...
    static decltype(IDirect3DDevice9Vtbl::Reset) __thimgui_dx9_reset_orig = nullptr;
    ...
    bool ImplDX9UnHookReset()
    {
        ...
        g_pd3dDevice->lpVtbl->Reset = __thimgui_dx9_reset_orig;
        __thimgui_dx9_reset_orig = nullptr;
        VirtualProtect(&g_pd3dDevice->lpVtbl->Reset, sizeof(void*), oldProt, &oldProt);
        return true;
    }
}
