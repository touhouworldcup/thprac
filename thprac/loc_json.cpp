...
void write_autogenerated_warning(string& output) {
	...
	sprintf_append(
		output,
		"// Then, when you rebuild, this file will regenerate automatically" ENDL
	);
}
...
void loc_json(
	...
	generate_header_file(out_header, games);
	generate_source_file(out_source, games);
...
constexpr inline bool later(FILETIME first_, FILETIME second_) {
	ULARGE_INTEGER first = {
		.LowPart = first_.dwLowDateTime,
		.HighPart = first_.dwHighDateTime,
	};

	ULARGE_INTEGER second = {
		.LowPart = second_.dwLowDateTime,
		.HighPart = second_.dwHighDateTime,
	};
...
int wmain(int argc, wchar_t** argv) {
	...
	if (!(GetFileAttributesW(src_folder) & FILE_ATTRIBUTE_DIRECTORY)) {
		fwprintf(stderr, L"Error: %s is not a folder\n", src_folder);
		return -2;
	}
	SetCurrentDirectoryW(src_folder);

	bool files_dont_exist = false;

	HANDLE hHeader = CreateFileW(L"" THPRAC_LOC_HEADER_NAME, GENERIC_WRITE, FILE_SHARE_READ, NULL, OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
	if (hHeader == INVALID_HANDLE_VALUE) {
		fprintf(stderr, "Error: failed to open " THPRAC_LOC_HEADER_NAME " for writing, code = %lu\n", GetLastError());
		return -7;
	}
	
	if(!GetLastError()) {
		files_dont_exist = true;
	}

	HANDLE hSource = CreateFileW(L"" THPRAC_LOC_SOURCE_NAME, GENERIC_WRITE, FILE_SHARE_READ, NULL, OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
	if (hSource == INVALID_HANDLE_VALUE) {
		fprintf(stderr, "Error: failed to open " THPRAC_LOC_SOURCE_NAME " for writing, code = %lu\n", GetLastError());
		CloseHandle(hHeader);
		return -7;
	}
	...
	FILETIME source_write_time, header_write_time;

	if (!GetFileTime(hHeader, NULL, NULL, &header_write_time)) {
		fwprintf(stderr, L"Error: failed to get last write time of " THPRAC_LOC_SOURCE_NAME ", code = %lu\n", GetLastError());
		CloseHandle(hHeader);
		CloseHandle(hSource);
		return -8;
	}
	if (!GetFileTime(hSource, NULL, NULL, &source_write_time)) {
		fwprintf(stderr, L"Error: failed to get last write time of " THPRAC_LOC_SOURCE_NAME ", code = %lu\n", GetLastError());
		CloseHandle(hHeader);
		CloseHandle(hSource);
		return -8;
	...
	FILETIME json_write_time;

	{
		MappedFile json_file(L"" THPRAC_LOC_JSON_NAME);
		if (!json_file.fileMapView) {
			fprintf(stderr, "Error: failed to map " THPRAC_LOC_JSON_NAME ", code = %lu\n", GetLastError());
			return -3;
		}
		if (!json_file.fileMap) {
			fprintf(stderr, "Error: failed create file mapping for " THPRAC_LOC_JSON_NAME ", code = %lu\n", GetLastError());
			return -3;
		}
		if (json_file.hFile == INVALID_HANDLE_VALUE) {
			fprintf(stderr, "Error: failed to open " THPRAC_LOC_JSON_NAME " for reading, code = %lu\n", GetLastError());
			return -4;
		}

		if (!GetFileTime(json_file.hFile, NULL, NULL, &json_write_time)) {
			fwprintf(stderr, L"Error: failed to get last write time of " THPRAC_LOC_JSON_NAME ", code = %lu", GetLastError());
			return -5;
		}

		d.Parse((char*)json_file.fileMapView, json_file.fileSize);
	}
	...
	if (files_dont_exist || later(json_write_time, source_write_time) || later(json_write_time, header_write_time)) {
		DWORD byteRet;

		std::string out_source, out_header;
		loc_json(d, out_source, out_header);

		SetEndOfFile(hSource);
		WriteFile(hSource, out_source.data(), out_source.size(), &byteRet, NULL);
		FlushFileBuffers(hSource);
		wprintf(L"" THPRAC_LOC_SOURCE_NAME ": wrote %d/%d bytes, code = %lu\n", byteRet, out_source.size(), GetLastError());

		SetEndOfFile(hHeader);
		WriteFile(hHeader, out_header.data(), out_header.size(), &byteRet, NULL);
		FlushFileBuffers(hHeader);
		wprintf(L"" THPRAC_LOC_HEADER_NAME ": wrote %d/%d bytes, code = %lu\n", byteRet, out_header.size(), GetLastError());
	}
