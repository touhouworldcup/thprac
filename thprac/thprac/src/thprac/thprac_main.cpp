...
THGameSig* CheckOngoingGame(PROCESSENTRY32& proc)
{
    ...
    if (strcmp("ìåï˚çgñÇãΩ.exe", proc.szExeFile) && strcmp("ñ|∑Ωºtƒß‡_.exe", proc.szExeFile)) {
...
bool CheckIfGameExist(THGameSig& gameSig, std::wstring& name)
{
    if (!strcmp(gameSig.idStr, "th06")) {
        if (CheckIfGameExistEx(gameSig, L"ìåï˚çgñÇãΩ.exe")) {
            name = L"ìåï˚çgñÇãΩ.exe";
            return true;
        } else if (CheckIfGameExistEx(gameSig, L"ñ|∑Ωºtƒß‡_.exe")) {
            name = L"ñ|∑Ωºtƒß‡_.exe";
            return true;
        }
    }
