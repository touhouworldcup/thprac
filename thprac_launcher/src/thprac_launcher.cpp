...
static void LauncherSettingsMain(LauncherState* state) {
    ...
    if (t) {
        ...
        if (ImGui::Button(S(THPRAC_THCRAP_LAUNCH_CONFIGURE))) {
            ...
            if (t.w_len + conf_exe.length() < MAX_PATH) {
                ...
                if ((UINT_PTR)ShellExecuteW(ImGui_ImplWin32_GetHwnd(), L"open", conf_path, nullptr, nullptr, SW_SHOW) > 32) {
                    state->thcrapError = A0000ERROR_C;
                } else {
        ...
        if (ImGui::Button(S(THPRAC_THCRAP_GET))) {
            ShellExecuteW(ImGui_ImplWin32_GetHwnd(), L"open", L"https://thpatch.net", nullptr, nullptr, SW_SHOW);
            state->thcrapError = A0000ERROR_C;
        }
        ImGui::SameLine();
        if (ImGui::Button(S(THPRAC_THCRAP_SET))) {
            std::wstring new_thcrap_dir;
            if (SelectFolder(new_thcrap_dir, ImGui_ImplWin32_GetHwnd())) {
    ...
    if (ImGui::Button(S(TH_VISIT_WEBSITE))) {
        ShellExecuteW(ImGui_ImplWin32_GetHwnd(), L"open", L"https://github.com/touhouworldcup/thprac", nullptr, nullptr, SW_SHOW);
    }
    ImGui::SameLine();
...
void UiUpdate(HWND hwnd, LauncherState* state) {
    ...
    if (Gui::Modal(S(THPRAC_UPDATE_MODAL))) {
        if (state->hUpdateThread) {
            ...
            do_the_update:
                ...
                break;
            case 1:
                ShellExecuteA(ImGui_ImplWin32_GetHwnd(), "open", background_update_check->updateJson.url, nullptr, nullptr, SW_SHOW);
            case -1:
            ...
            case 0:
                ShellExecuteA(ImGui_ImplWin32_GetHwnd(), "open", background_update_check->updateJson.changelog, nullptr, nullptr, SW_SHOW);
                break;
            case 1:
                ImGui::CloseCurrentPopup();
            }
    ...
    auto* dev = ImGui_ImplDX9_GetDevice();
...
int Launcher(HINSTANCE hInstance, int nCmdShow) {
    ...
    // THPrac's ImGui_ImplWin32_NewFrame removed that line because
...
LRESULT WINAPI WndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_SIZE:
        if (ImGui_ImplDX9_GetDevice() != NULL && wParam != SIZE_MINIMIZED) {
...
void ResetDevice() {
    ...
    HRESULT hr = ImGui_ImplDX9_GetDevice()->Reset(&g_d3dpp);
