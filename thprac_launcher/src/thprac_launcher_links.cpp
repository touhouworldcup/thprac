...
int EditLinkUI(char* linkEditTitleBuf, char* linkEditLinkBuf, bool& linkNameWarn, bool& linkLinkWarn) {
    ...
    if (ImGui::Button(S(THPRAC_LINKS_EDIT_FILE))) {
        ...
        OPENFILENAMEW ofn = {
            ...
            .hwndOwner = ImGui_ImplWin32_GetHwnd(),
            .lpstrFile = szFile,
            .nMaxFile = MAX_PATH + 1,
    ...
    if (ImGui::Button(S(THPRAC_LINKS_EDIT_FOLDER))) {
        std::wstring folder;
        if (SelectFolder(folder, ImGui_ImplWin32_GetHwnd())) {
        ...
        if (nodeOpen) {
            ...
            for (size_t linkIdx = 0; linkIdx < state->linkSets[linkSetIdx].links.size(); linkIdx++) {
                ...
                if (ImGui::IsItemHovered() && ImGui::IsMouseDoubleClicked(ImGuiMouseButton_Left)) {
                    ...
                    if ((UINT_PTR)ShellExecuteW(ImGui_ImplWin32_GetHwnd(), L"open", u16_cmd.c_str(), nullptr, nullptr, SW_SHOW) < 32) {
