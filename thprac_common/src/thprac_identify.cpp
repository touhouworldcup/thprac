

#include "thprac_identify.h"
#include "thprac_utils.h"
#include "utils.h"

#include <metrohash128.h>

void DummyInit() {
    __debugbreak();
}

TH_INIT_FUNC_32_DECL(TH06Init);
TH_INIT_FUNC_64_DECL(TH06NCInit);
TH_INIT_FUNC_32_DECL(TH07Init);
TH_INIT_FUNC_32_DECL(TH08Init);
TH_INIT_FUNC_32_DECL(TH09Init);
TH_INIT_FUNC_32_DECL(TH095Init);
TH_INIT_FUNC_32_DECL(TH10Init);
TH_INIT_FUNC_32_DECL(AlcostgInit);
TH_INIT_FUNC_32_DECL(TH11Init);
TH_INIT_FUNC_32_DECL(TH12Init);
TH_INIT_FUNC_32_DECL(TH125Init);
TH_INIT_FUNC_32_DECL(TH128Init);
TH_INIT_FUNC_32_DECL(TH13Init);
TH_INIT_FUNC_32_DECL(TH14Init);
TH_INIT_FUNC_32_DECL(TH143Init);
TH_INIT_FUNC_32_DECL(TH15Init);
TH_INIT_FUNC_32_DECL(TH16Init);
TH_INIT_FUNC_32_DECL(TH165Init);
TH_INIT_FUNC_32_DECL(TH17Init);
TH_INIT_FUNC_32_DECL(TH18Init);
TH_INIT_FUNC_32_DECL(TH185Init);
TH_INIT_FUNC_32_DECL(TH19_v1_00a_Init);
TH_INIT_FUNC_32_DECL(TH19_v1_10c_Init);
TH_INIT_FUNC_32_DECL(TH20Init);

// Putting them into an array, then having an enum to label indicies into this array ensures that all of these end up being sequential in memory
// Multiple versions of the same game MUST be sequential in this array. New versions must always be inserted after the last added existing version
// so if you add th15 v1.00a for example, it must come after th15 v1.00b, which is the currently supported version of th15.
constexpr const THGameVersion gGameVersions[] = {
    {
        .gameId = ID_ALCOSTG,
        .initFunc = TH_INIT_FUNC_32(AlcostgInit),
        .has_vpatch = true,
        .has_oilp = false,
        .exeInfo = {
            .timeStamp = 1198873702,
            .textSize = 397312,
        },
        .oepCode = { 0x212b, 0x322a, 0x0524, 0xac44, 0x5691, 0x4646, 0xd3f8, 0x4848, 0xc249, 0xa28d }
    },
    {
        .gameId = ID_TH06,
        .initFunc = TH_INIT_FUNC_32(TH06Init),
        .has_vpatch = true,
        .has_oilp = true,
        .exeInfo = {
            .timeStamp = 1038721275,
            .textSize = 430080,
        },
        .oepCode = { 0x212b, 0xe22a, 0x05bc, 0xac44, 0x4867, 0x4646, 0xd3f8, 0x4848, 0xc249, 0xa28d }
    },
    {
        .gameId = ID_TH06NC,
        .initFunc = TH_INIT_FUNC_64(TH06NCInit),
        .has_vpatch = false,
        .has_oilp = false,
        .exeInfo = {
            .timeStamp = 0x6a9e3a91,
            .textSize = 0x002bec00
        },
        .oepCode = {0xc209, 0x6aae, 0x00ab, 0x4442, 0x0d45, 0x82c5, 0xae6f, 0xb632, 0xb6b6, 0x8686},
    },
    {
        .gameId = ID_TH07,
        .initFunc = TH_INIT_FUNC_32(TH07Init),
        .has_vpatch = true,
        .has_oilp   = true,
        .exeInfo = {
            .timeStamp = 1066942337,
            .textSize = 572928,
        },
        .oepCode = { 0x212b, 0xe22a, 0x0a45, 0xac44, 0x5ede, 0x4646, 0xd3f8, 0x4848, 0xc249, 0xa28d }
    },
    {
        .gameId = ID_TH075,
        .initFunc = TH_INIT_FUNC_NULL,
        .has_vpatch = false,
        .has_oilp   = false,
        .exeInfo = {
            .timeStamp = 1120788294,
            .textSize = 2449408,
        },
        .oepCode = { 0x212b, 0x3a2a, 0x254c, 0xac44, 0x7599, 0x4646, 0xd3f8, 0x4848, 0xc249, 0xa28d }
    },
    {
        .gameId = ID_TH08,
        .initFunc = TH_INIT_FUNC_32(TH08Init),
        .has_vpatch = true,
        .has_oilp   = true,
        .exeInfo = {
            .timeStamp = 1095610188,
            .textSize = 728064,
        },
        .oepCode = { 0x212b, 0xf22a, 0x089b, 0xac44, 0x498b, 0x4646, 0xd3f8, 0x4848, 0xc249, 0xa28d }
    },
    {
        .gameId = ID_TH09,
        .initFunc = TH_INIT_FUNC_32(TH09Init),
        .has_vpatch = true,
        .has_oilp   = true,
        .exeInfo = {
            .timeStamp = 1128708539,
            .textSize = 576512,
        },
        .oepCode = { 0x212b, 0xea2a, 0x0a16, 0xac44, 0x5900, 0x4646, 0xd3f8, 0x4848, 0xc249, 0xa28d }
    },
    {  // v1.00a
        .gameId = ID_TH09,
        .initFunc = TH_INIT_FUNC_NULL,
        .has_vpatch = true,
        .has_oilp   = true,
        .exeInfo = {
            .timeStamp = 1122350436,
            .textSize = 556544,
        },
        .oepCode = { 0x212b, 0xca2a, 0x0a41, 0xac44, 0x5900, 0x4646, 0xd3f8, 0x4848, 0xc249, 0xa28d }
    },
    {
        .gameId = ID_TH095,
        .initFunc = TH_INIT_FUNC_32(TH095Init),
        .has_vpatch = true,
        .has_oilp   = true,
        .exeInfo = {
            .timeStamp = 1137085759,
            .textSize = 603136,
        },
        .oepCode = { 0x212b, 0x122a, 0x0af8, 0xac44, 0x5ce2, 0x4646, 0xd3f8, 0x4848, 0xc249, 0xa28d }
    },
    {
        .gameId = ID_TH10,
        .initFunc = TH_INIT_FUNC_32(TH10Init),
        .has_vpatch = true,
        .has_oilp   = true,
        .exeInfo = {
            .timeStamp = 1186086096,
            .textSize = 412672,
        },
        .oepCode = { 0x212b, 0x322a, 0x05e4, 0xac44, 0x5691, 0x4646, 0xd3f8, 0x4848, 0xc249, 0xa28d }
    },
    {
        .gameId = ID_TH105,
        .initFunc = TH_INIT_FUNC_NULL,
        .has_vpatch = false,
        .has_oilp   = false,
        .exeInfo = {
            .timeStamp = 1319458676,
            .textSize = 2879488,
        },
        .oepCode = { 0x8ba9, 0x42dd, 0xaa43, 0xba52, 0xbaba, 0x8a8a, 0x8b8b, 0x0ac5, 0x12b6, 0xc789 }
    },
    {
        .gameId = ID_TH11,
        .initFunc = TH_INIT_FUNC_32(TH11Init),
        .has_vpatch = true,
        .has_oilp   = true,
        .exeInfo = {
            .timeStamp = 1217734394,
            .textSize = 565248,
        },
        .oepCode = { 0x06a9, 0x4289, 0xaa43, 0xba3c, 0xbaba, 0xb9cd, 0xcc12, 0xc9a4, 0x61a5, 0x4a49 }
    },
    {
        .gameId = ID_TH12,
        .initFunc = TH_INIT_FUNC_32(TH12Init),
        .has_vpatch = true,
        .has_oilp   = true,
        .exeInfo = {
            .timeStamp = 1251344348,
            .textSize = 617472,
        },
        .oepCode = { 0x06a9, 0x4289, 0xaa43, 0xba3c, 0xbaba, 0xb9cd, 0xcc12, 0xc9a4, 0x61a5, 0x4a49 }
    },
    {
        .gameId = ID_TH123,
        .initFunc = TH_INIT_FUNC_NULL,
        .has_vpatch = false,
        .has_oilp   = false,
        .exeInfo = {
            .timeStamp = 1316148875,
            .textSize = 4546560,
        },
        .oepCode = { 0xa8a9, 0x42ec, 0xaa43, 0xba52, 0xbaba, 0x8a8a, 0x8b8b, 0x0ac5, 0x12b6, 0xc789 }
    },
    {
        .gameId = ID_TH125,
        .initFunc = TH_INIT_FUNC_32(TH125Init),
        .has_vpatch = true,
        .has_oilp   = true,
        .exeInfo = {
            .timeStamp = 1267822137,
            .textSize = 611328,
        },
        .oepCode = { 0xf8a9, 0x42f9, 0xaa43, 0xba3c, 0xbaba, 0xb9cd, 0xcc12, 0xc9a4, 0x61a5, 0x4a49 }
    },
    {
        .gameId = ID_TH128,
        .initFunc = TH_INIT_FUNC_32(TH128Init),
        .has_vpatch = true,
        .has_oilp   = true,
        .exeInfo = {
            .timeStamp = 1280811414,
            .textSize = 626176,
        },
        .oepCode = { 0x03a9, 0x42dd, 0xaa43, 0xbacd, 0xbaba, 0xb9cd, 0xcc12, 0xcba4, 0x69a5, 0x0fc1 }
    },
    {
        .gameId = ID_TH13,
        .initFunc = TH_INIT_FUNC_32(TH13Init),
        .has_vpatch = true,
        .has_oilp   = true,
        .exeInfo = {
            .timeStamp = 1313589413,
            .textSize = 656384,
        },
        .oepCode = { 0xb6a9, 0x42dc, 0xaa43, 0xbacd, 0xbaba, 0xb9cd, 0xcc12, 0xcba4, 0x69a5, 0x0fc1 }
    },
    {
        .gameId = ID_TH135,
        .initFunc = TH_INIT_FUNC_NULL,
        .has_vpatch = false,
        .has_oilp   = false,
        .exeInfo = {
            .timeStamp = 1381475275,
            .textSize = 3928064,
        },
        .oepCode = { 0xd7a9, 0x4340, 0xaa43, 0xbacd, 0xbaba, 0xb9cd, 0xcc12, 0xc3a4, 0x410c, 0x16e9 }
    },
    {
        .gameId = ID_TH14,
        .initFunc = TH_INIT_FUNC_32(TH14Init),
        .has_vpatch = true, // As much as I hate to admit to it
        .has_oilp   = true,
        .exeInfo = {
            .timeStamp = 1376634201,
            .textSize = 720896,
        },
        .oepCode = { 0xafa9, 0x42f5, 0xaa43, 0x4444, 0x4545, 0x522c, 0x1f2f, 0x0486, 0xa149, 0x511d }
    },
    {
        .gameId = ID_TH143,
        .initFunc = TH_INIT_FUNC_32(TH143Init),
        .has_vpatch = false,
        .has_oilp   = true,
        .exeInfo = {
            .timeStamp = 1398039605,
            .textSize = 748032,
        },
        .oepCode = { 0x6aa9, 0x428b, 0xaa43, 0x4444, 0x4545, 0x522c, 0xdf2f, 0x0511, 0xa149, 0x56f7 }
    },
    {
        .gameId = ID_TH145,
        .initFunc = TH_INIT_FUNC_NULL,
        .has_vpatch = false,
        .has_oilp   = false,
        .exeInfo = {
            .timeStamp = 1492674616,
            .textSize = 3516928,
        },
        .oepCode = { 0x83a9, 0x424d, 0xaa43, 0xbaca, 0xbaba, 0x562c, 0x772f, 0xcb28, 0xa149, 0x4500 }
    },
    {
        .gameId = ID_TH15,
        .initFunc = TH_INIT_FUNC_32(TH15Init),
        .has_vpatch = true,
        .has_oilp   = true,
        .exeInfo = {
            .timeStamp = 1444028071,
            .textSize = 772608,
        },
        .oepCode = { 0x1aa9, 0x428b, 0xaa43, 0x4444, 0x4545, 0x522c, 0x872f, 0x05f1, 0xa149, 0x56f7 }
    },
    {
        .gameId = ID_TH155,
        .initFunc = TH_INIT_FUNC_NULL,
        .has_vpatch = false,
        .has_oilp   = false,
        .exeInfo = {
            .timeStamp = 1524477382,
            .textSize = 3695616,
        },
        .oepCode = { 0x37a9, 0x424c, 0xaa43, 0xbaca, 0xbaba, 0x562c, 0xb72f, 0xc163, 0xa149, 0x47b4 }
    },
    {
        .gameId = ID_TH16,
        .initFunc = TH_INIT_FUNC_32(TH16Init),
        .has_vpatch = false,
        .has_oilp   = true,
        .exeInfo = {
            .timeStamp = 1501534029,
            .textSize = 564736,
        },
        .oepCode = { 0xeaa9, 0x4246, 0xaa43, 0xbaca, 0xbaba, 0xcd13, 0x11ab, 0x3db7, 0xc241, 0xa2bb }
    },
    {
        .gameId = ID_TH165,
        .initFunc = TH_INIT_FUNC_32(TH165Init),
        .has_vpatch = false,
        .has_oilp   = true,
        .exeInfo = {
            .timeStamp = 1532998383,
            .textSize = 607232,
        },
        .oepCode = { 0xb9a9, 0x4246, 0xaa43, 0xbac0, 0xbaba, 0xcd13, 0x11ab, 0x3db7, 0xc241, 0xa2bb }
    },
    {
        .gameId = ID_TH17,
        .initFunc = TH_INIT_FUNC_32(TH17Init),
        .has_vpatch = false,
        .has_oilp   = true,
        .exeInfo = {
            .timeStamp = 1565429804,
            .textSize = 626688,
        },
        .oepCode = { 0xb2a9, 0x4246, 0xaa43, 0xba3e, 0xbaba, 0xcd13, 0x11ab, 0x3db7, 0xc241, 0xa2bb }
    },
    {  // v1.01 
        .gameId = ID_TH175,
        .initFunc = TH_INIT_FUNC_NULL,
        .has_vpatch = false,
        .has_oilp   = false,
        .exeInfo = {
            .timeStamp = 1635319660,
            .textSize = 375296,
        },
        .oepCode = { 0x31a9, 0x4244, 0xaa43, 0xbaca, 0xbaba, 0xcd13, 0x11ab, 0x3db7, 0xc241, 0xa2bb }
    },
    {  // Latest Steam version
        .gameId = ID_TH175,
        .initFunc = TH_INIT_FUNC_NULL,
        .has_vpatch = false,
        .has_oilp   = false,
        .exeInfo = {
            .timeStamp = 1675058423,
            .textSize = 414208,
        },
        .oepCode = { 0x31a9, 0x4244, 0xaa43, 0xbaca, 0xbaba, 0xcd13, 0x11ab, 0x3db7, 0xc241, 0xa2bb }
    },
    {
        .gameId = ID_TH18,
        .initFunc = TH_INIT_FUNC_32(TH18Init),
        .has_vpatch = false,
        .has_oilp   = true,
        .exeInfo = {
            .timeStamp = 1618619144,
            .textSize = 702464,
        },
        .oepCode = { 0x9ba9, 0x4246, 0xaa43, 0xba3e, 0xbaba, 0xcd13, 0xc4ab, 0x44a4, 0x04c4, 0xa2be }
    },
    {
        .gameId = ID_TH185,
        .initFunc = TH_INIT_FUNC_32(TH185Init),
        .has_vpatch = false,
        .has_oilp   = false,
        .exeInfo = {
            .timeStamp = 1659144319,
            .textSize = 711168,
        },
        .oepCode = { 0x84a9, 0x4246, 0xaa43, 0xba3e, 0xbaba, 0xcd13, 0xc4ab, 0x44a4, 0x04c4, 0xa2be }
    },
    {
        .gameId = ID_TH19,
        .initFunc = TH_INIT_FUNC_32(TH19_v1_00a_Init),
        .has_vpatch = false,
        .has_oilp   = false,
        .exeInfo = {
            .timeStamp = 1690598468,
            .textSize = 1433600,
        },
        .oepCode = { 0xdfa9, 0x4247, 0xaa43, 0xba3e, 0xbaba, 0x0e2e, 0x1d90, 0xb748, 0x8d5c, 0x1fbb }
    },
    {
        .gameId = ID_TH19,
        .initFunc = TH_INIT_FUNC_32(TH19_v1_10c_Init),
        .has_vpatch = false,
        .has_oilp   = false,
        .exeInfo = {
            .timeStamp = 1720429610,
            .textSize = 1544704,
        },
        .oepCode = { 0xb3a9, 0x4245, 0xaa43, 0xba3e, 0xbaba, 0x0e2e, 0x1a4a, 0xb748, 0xad5c, 0x1dfb }
    },
    {
        .gameId = ID_TH20,
        .initFunc = TH_INIT_FUNC_32(TH20Init),
        .has_vpatch = false,
        .has_oilp   = false,
        .exeInfo = {
            .timeStamp = 1753839620,
            .textSize = 1484288,
        },
        .oepCode = { 0xaca9, 0x4244, 0xaa43, 0xba33, 0xbaba, 0x4684, 0xcc47, 0xbc05, 0xc02d, 0x4a47 }
    }
};

static_assert(gGameVersions[VER_ALCOSTG].gameId == ID_ALCOSTG);
static_assert(gGameVersions[VER_TH06].gameId == ID_TH06);
static_assert(gGameVersions[VER_TH06NC].gameId == ID_TH06NC);
static_assert(gGameVersions[VER_TH07].gameId == ID_TH07);
static_assert(gGameVersions[VER_TH075].gameId == ID_TH075);
static_assert(gGameVersions[VER_TH08].gameId == ID_TH08);
static_assert(gGameVersions[VER_TH09].gameId == ID_TH09);
static_assert(gGameVersions[VER_TH09_V1_00A].gameId == ID_TH09);
static_assert(gGameVersions[VER_TH095].gameId == ID_TH095);
static_assert(gGameVersions[VER_TH10].gameId == ID_TH10);
static_assert(gGameVersions[VER_TH105].gameId == ID_TH105);
static_assert(gGameVersions[VER_TH11].gameId == ID_TH11);
static_assert(gGameVersions[VER_TH12].gameId == ID_TH12);
static_assert(gGameVersions[VER_TH123].gameId == ID_TH123);
static_assert(gGameVersions[VER_TH125].gameId == ID_TH125);
static_assert(gGameVersions[VER_TH128].gameId == ID_TH128);
static_assert(gGameVersions[VER_TH13].gameId == ID_TH13);
static_assert(gGameVersions[VER_TH135].gameId == ID_TH135);
static_assert(gGameVersions[VER_TH14].gameId == ID_TH14);
static_assert(gGameVersions[VER_TH143].gameId == ID_TH143);
static_assert(gGameVersions[VER_TH145].gameId == ID_TH145);
static_assert(gGameVersions[VER_TH15].gameId == ID_TH15);
static_assert(gGameVersions[VER_TH155].gameId == ID_TH155);
static_assert(gGameVersions[VER_TH16].gameId == ID_TH16);
static_assert(gGameVersions[VER_TH165].gameId == ID_TH165);
static_assert(gGameVersions[VER_TH17].gameId == ID_TH17);
static_assert(gGameVersions[VER_TH175].gameId == ID_TH175);
static_assert(gGameVersions[VER_TH175_V1_15].gameId == ID_TH175);
static_assert(gGameVersions[VER_TH18].gameId == ID_TH18);
static_assert(gGameVersions[VER_TH185].gameId == ID_TH185);
static_assert(gGameVersions[VER_TH19_V1_00A].gameId == ID_TH19);
static_assert(gGameVersions[VER_TH19_V1_10C].gameId == ID_TH19);
static_assert(gGameVersions[VER_TH20].gameId == ID_TH20);

const unsigned int gGameVersionsCount = elementsof(gGameVersions);

const THKnownGame gKnownGames[] = {
    { gGameVersions + VER_ALCOSTG,
        TYPE_ORIGINAL,
        { 0x3d65fad0, 0xd4c4e831,
            0x4fb58774, 0x957f0157 } },
    { gGameVersions + VER_TH06,
        TYPE_ORIGINAL,
        { 0xbae18847, 0x78d78ce2,
            0x4d19703a, 0x3f5cbe16 } },
    { gGameVersions + VER_TH06,
        TYPE_CHINESE,
        { 0x6c2e0eb5, 0x6bb9bce8,
            0x4ffcf20e, 0x65e9c0bf } },
    { gGameVersions + VER_TH06,
        TYPE_ORIGINAL,
        { 0xd179f6c8, 0x0af9170,
            0xae99b942, 0x4c974b53 } },
    { gGameVersions + VER_TH06NC,
        TYPE_ORIGINAL,
        { 0x2e28dca2, 0x7645dff5, 
            0x947defe8, 0xb3bc0d1f }},
    { gGameVersions + VER_TH07,
        TYPE_ORIGINAL,
        { 0xd69a5df6, 0x4a78f383,
            0x956d7f9f, 0x02e3ad3b } },
    { gGameVersions + VER_TH07,
        TYPE_SCHINESE,
        { 0x8995e319, 0x17c3ee1c,
            0x6034a965, 0x70fefd77 } },
    { gGameVersions + VER_TH07,
        TYPE_SCHINESE,
        { 0xdc5ae3c0, 0x959663c2,
            0x5cebd278, 0x956424fe } },
    { gGameVersions + VER_TH075,
        TYPE_ORIGINAL,
        { 0xececd7e6, 0xc7e6b23e,
            0xd87c04f6, 0x98c496dc } },
    { gGameVersions + VER_TH075,
        TYPE_SCHINESE,
        { 0xbdcd7982, 0x41103975,
            0x75dc169d, 0x88298932 } },
    { gGameVersions + VER_TH08,
        TYPE_ORIGINAL,
        { 0xc1f84aea, 0xfad04d3b,
            0xf28642e5, 0x7fe28f3b } },
    { gGameVersions + VER_TH08,
        TYPE_SCHINESE,
        { 0x58db89b4, 0x5a9fe98a,
            0x2dea944f, 0xaebe92d9 } },
    { gGameVersions + VER_TH09,
        TYPE_ORIGINAL,
        { 0xdc63c169, 0x2c975430,
            0xbc21df80, 0x26937f9e } },
    { gGameVersions + VER_TH09,
        TYPE_SCHINESE,
        { 0x479728a5, 0xefc341e0,
            0x260d5e91, 0xe6851479 } },
    { gGameVersions + VER_TH09_V1_00A,
        TYPE_ORIGINAL,
        { 0x1f970004, 0x5b12aa84,
            0xee19f17, 0xd7fd7729 } },
    { gGameVersions + VER_TH095,
        TYPE_ORIGINAL,
        { 0x46eaf90e, 0x2a24f234,
            0x74b70f03, 0xbd59c6e5 } },
    { gGameVersions + VER_TH095,
        TYPE_STEAM,
        { 0x9e7883da, 0x3e8a7a62,
            0x333fa441, 0x9e4a5077 } },
    { gGameVersions + VER_TH095,
        TYPE_SCHINESE,
        { 0xc0ba2aae, 0xd037ee77,
            0xda79797d, 0x53a7ebc1 } },
    { gGameVersions + VER_TH10,
        TYPE_ORIGINAL,
        { 0x11c73117, 0x422a725f,
            0xc0639015, 0x06050767 } },
    { gGameVersions + VER_TH10,
        TYPE_STEAM,
        { 0x4771b7b2, 0x61a9b6ff,
            0x77b2d73c, 0xb05af556 } },
    { gGameVersions + VER_TH10,
        TYPE_SCHINESE,
        { 0x2680e8df, 0xe528eef4,
            0x2a07c721, 0x6bc772df } },
    { gGameVersions + VER_TH10,
        TYPE_TCHINESE,
        { 0x03a4114f, 0xa6bfb0d7,
            0xcbc29fec, 0x7028a7b1 } },
    { gGameVersions + VER_TH105,
        TYPE_ORIGINAL,
        { 0x31aaf075, 0x7fe79569,
            0x5e0ad6ea, 0x8105772e } },
    { gGameVersions + VER_TH105,
        TYPE_SCHINESE,
        { 0x6a09d7da, 0xe25c8d1b,
            0xdfa00b4f, 0x1fe8bb5f } },
    { gGameVersions + VER_TH11,
        TYPE_ORIGINAL,
        { 0x4e4ad931, 0xa91dc711,
            0x720e5db7, 0x57a14232 } },
    { gGameVersions + VER_TH11,
        TYPE_STEAM,
        { 0xa6c81bda, 0xab6fea4b,
            0xb9708f50, 0x31242cdd } },
    { gGameVersions + VER_TH11,
        TYPE_SCHINESE,
        { 0xca516350, 0x63057ef3,
            0x815a096e, 0x50afd6b8 } },
    { gGameVersions + VER_TH12,
        TYPE_ORIGINAL,
        { 0xd4d81259, 0xce424514,
            0x6e7e6326, 0x8b4c0990 } },
    { gGameVersions + VER_TH12,
        TYPE_STEAM,
        { 0x36865d01, 0x4402f03e,
            0x8d44dabd, 0x65281618 } },
    { gGameVersions + VER_TH12,
        TYPE_SCHINESE,
        { 0xddf5470b, 0xe89d1019,
            0x4fb6bf56, 0x738153ad } },
    { gGameVersions + VER_TH123,
        TYPE_ORIGINAL,
        { 0x7be66ad2, 0x4a21e375,
            0xc7d08b4f, 0x7cc3c681 } },
    { gGameVersions + VER_TH123,
        TYPE_SCHINESE,
        { 0x2a7099d1, 0xf0244e40,
            0x8fcab54c, 0xbdba4a5d } },
    { gGameVersions + VER_TH125,
        TYPE_ORIGINAL,
        { 0x0e1b7b17, 0x473f857e,
            0x3fe6360f, 0x40de70d3 } },
    { gGameVersions + VER_TH125,
        TYPE_STEAM,
        { 0x88d8f1d8, 0x4820a04b,
            0x4414c45d, 0xcfaa2117 } },
    { gGameVersions + VER_TH125,
        TYPE_SCHINESE,
        { 0x66928829, 0x80da58eb,
            0xd9dfd4da, 0x8333fcce } },
    { gGameVersions + VER_TH128,
        TYPE_ORIGINAL,
        { 0x948a4e8a, 0x665d6cd0,
            0x25fc26f7, 0xcba8f1e3 } },
    { gGameVersions + VER_TH128,
        TYPE_STEAM,
        { 0x48b1d757, 0x29766b53,
            0xc07d3645, 0x7d535f20 } },
    { gGameVersions + VER_TH128,
        TYPE_SCHINESE,
        { 0x126829c6, 0xd6eab530,
            0xf9734a83, 0xee657af5 } },
    { gGameVersions + VER_TH13,
        TYPE_ORIGINAL,
        { 0x4619502d, 0xe94742fc,
            0x13537c7d, 0x7212f384 } },
    { gGameVersions + VER_TH13,
        TYPE_STEAM,
        { 0xd60fa763, 0x380673c1,
            0x3a3f6475, 0x11dc1f00 } },
    { gGameVersions + VER_TH13,
        TYPE_ORIGINAL,
        { 0xb74eefbb, 0x7a38aa6f,
            0xfa5258ff, 0x81b1c16c } },
    { gGameVersions + VER_TH13,
        TYPE_SCHINESE,
        { 0xbfaa3ea8, 0x1242e9dd,
            0x82953fb0, 0x40d24284 } },
    { gGameVersions + VER_TH13,
        TYPE_SCHINESE,
        { 0x7c12239d, 0xf9e1b1a5,
            0x2388ce13, 0x7e458f48 } },
    { gGameVersions + VER_TH135,
        TYPE_SCHINESE,
        { 0xf2246b98, 0x9d4a7bf0,
            0x2c1d8f07, 0x84bc1a99 } },
    { gGameVersions + VER_TH14,
        TYPE_ORIGINAL,
        { 0x08a6fde4, 0xb4344a4a,
            0xa3636647, 0x387d4253 } },
    { gGameVersions + VER_TH14,
        TYPE_STEAM,
        { 0x5edd7a93, 0x774d84b,
            0x5df1b298, 0x210f5fc8 } },
    { gGameVersions + VER_TH14,
        TYPE_NYASAMA,
        { 0xdc9235f5, 0xe124c565,
            0x6485e269, 0xf4686950 } },
    { gGameVersions + VER_TH143,
        TYPE_ORIGINAL,
        { 0x2549f2c0, 0x1121c489,
            0x9aaf6330, 0x75ade252 } },
    { gGameVersions + VER_TH143,
        TYPE_STEAM,
        { 0x227e06a2, 0x722509d9,
            0x01718a61, 0xad905c25 } },
    { gGameVersions + VER_TH143,
        TYPE_NYASAMA,
        { 0x47cdb883, 0xbe3921ae,
            0xf9c0c825, 0x2cefb060 } },
    { gGameVersions + VER_TH145,
        TYPE_ORIGINAL,
        { 0x416e1a70, 0xcdae8d4f,
            0xe62060d0, 0xc592d164 } },
    { gGameVersions + VER_TH145,
        TYPE_SCHINESE,
        { 0x8c6911ea, 0xd4ab7879,
            0x82297970, 0xbfd67515 } },
    { gGameVersions + VER_TH15,
        TYPE_ORIGINAL,
        { 0xed3ac6ec, 0x21bc473c,
            0x186edbb4, 0x9ebd98cf } },
    { gGameVersions + VER_TH15,
        TYPE_STEAM,
        { 0x75871bd4, 0x0adeb360,
            0xf429659d, 0x931922e2 } },
    { gGameVersions + VER_TH15,
        TYPE_NYASAMA,
        { 0xcfe2e487, 0xc7e04af4,
            0x9160e8c1, 0x98291df2 } },
    { gGameVersions + VER_TH155,
        TYPE_ORIGINAL,
        { 0xa42ea27d, 0xf14ae534,
            0xde6c96e0, 0xb78ce61b } },
    { gGameVersions + VER_TH16,
        TYPE_ORIGINAL,
        { 0x4dae29e4, 0xa1af84f8,
            0xc6d1d694, 0x8cb29894 } },
    { gGameVersions + VER_TH16,
        TYPE_STEAM,
        { 0x60ebdd82, 0xc012c21a,
            0x9d875c04, 0x6c1f7fef } },
    { gGameVersions + VER_TH16,
        TYPE_NYASAMA,
        { 0xbf82bc22, 0xe19074c3,
            0x4fb9a1d7, 0xfa429c31 } },
    { gGameVersions + VER_TH165,
        TYPE_ORIGINAL,
        { 0x550d4b1c, 0x805408e3,
            0x1ef2ca25, 0xa257adb3 } },
    { gGameVersions + VER_TH165,
        TYPE_STEAM,
        { 0x886148be, 0x11ed264e,
            0x369d5e4b, 0x16045490 } },
    { gGameVersions + VER_TH165,
        TYPE_NYASAMA,
        { 0xfcdcbd55, 0x8b944a23,
            0x32f3f96d, 0xefb7750e } },
    { gGameVersions + VER_TH17,
        TYPE_ORIGINAL,
        { 0x3b46eeb2, 0xcd3b5634,
            0x3e1032a2, 0x22cac0c5 } },
    { gGameVersions + VER_TH17,
        TYPE_STEAM,
        { 0x9fb6f999, 0xcc31ff48,
            0x57271b1f, 0x82db4030 } },
    { gGameVersions + VER_TH17,
        TYPE_NYASAMA,
        { 0x2f370234, 0xff6024b0,
            0x80f25da9, 0xcf2e1bf4 } },
    { gGameVersions + VER_TH175,
        TYPE_ORIGINAL,
        { 0xac9cb8f9, 0x1972b2cc,
            0xadab5748, 0x751593e1 } },
    { gGameVersions + VER_TH175_V1_15,
        TYPE_ORIGINAL,
        { 0xc96cffd1, 0x88e7c0df,
            0x55fb0585, 0x3d899cfb } },
    { gGameVersions + VER_TH18,
        TYPE_ORIGINAL,
        { 0xa3f0a451, 0x85ce8668,
            0xfa4eda8a, 0xd808c596 } },
    { gGameVersions + VER_TH18,
        TYPE_NYASAMA,
        { 0x71115032, 0x3a4d05bd,
            0x8dfc8ba4, 0xa476dc6f } },
    { gGameVersions + VER_TH18,
        TYPE_STEAM,
        { 0x7b13a1f3, 0x3eb0e397,
            0xa6f5b6ec, 0xcbf302ce } },
    { gGameVersions + VER_TH185,
        TYPE_ORIGINAL,
        { 0x79cfcdbf, 0xfaa400a6,
            0x5f92bcd6, 0x65a0d95 } },
    { gGameVersions + VER_TH185,
        TYPE_STEAM,
        { 0xcd1d1ad0, 0x34cf8cc1,
            0x32af8b3a, 0xcb07b8dc } },
    { gGameVersions + VER_TH19_V1_00A,
        TYPE_ORIGINAL,
        { 0x1ef2d050, 0xbddd8da2,
            0x56cedb87, 0xe674cd2c } },
    { gGameVersions + VER_TH19_V1_00A,
        TYPE_STEAM,
        { 0xfeac2cc1, 0xff97767b,
            0x7760eca9, 0xa7f85003 } },
    { gGameVersions + VER_TH19_V1_10C,
        TYPE_ORIGINAL,
        { 0xc2d1ef12, 0x6aa14d56,
            0x172284a9, 0x1d4147d8 } },
    { gGameVersions + VER_TH19_V1_10C,
        TYPE_STEAM,
        { 0x4bb3eeda, 0xdb1651f5,
            0xf670bb28, 0x8aea075e } },
    { gGameVersions + VER_TH20,
        TYPE_ORIGINAL,
        { 0x6cc723ea, 0xa71db65e,
            0x3c19f37c, 0x836fb369 } },
    { gGameVersions + VER_TH20,
        TYPE_STEAM,
        { 0xc0744bc1, 0x3c2aea32,
            0x9241a33e, 0xc467ae44 } },
};
const unsigned int gKnownGamesCount = elementsof(gKnownGames);

const char* gThGameStrs[] = {
    "unknown",
    "alcostg",
    "th01",
    "th02",
    "th03",
    "th04",
    "th05",
    "th06",
    "th06nc",
    "th07",
    "th075",
    "th08",
    "th09",
    "th095",
    "th10",
    "th105",
    "th11",
    "th12",
    "th123",
    "th125",
    "th128",
    "th13",
    "th135",
    "th14",
    "th143",
    "th145",
    "th15",
    "th155",
    "th16",
    "th165",
    "th17",
    "th175",
    "th18",
    "th185",
    "th19",
    "th20",
};

ExeInfo GetExeInfo(const uint8_t* mod, size_t len) {
    ExeInfo out = {};

    // Dawg
    if (len && len < 64) {  // Make sure that the length is long enough to read the e_lfanew field
        return out;
    }

    if (mod[0] != 'M' || mod[1] != 'Z') {
        return out;
    }

    auto* dosHeader = (IMAGE_DOS_HEADER*)mod;   
    auto* ntHeader = (IMAGE_NT_HEADERS*)(mod + dosHeader->e_lfanew);

    CHKBUF(mod, (unsigned char*)ntHeader + sizeof(IMAGE_NT_HEADERS), len, out);

    if (ntHeader->Signature != TextInt('P', 'E')) {
        return out;
    }

    auto* section = (IMAGE_SECTION_HEADER*)((uintptr_t)&ntHeader->OptionalHeader + ntHeader->FileHeader.SizeOfOptionalHeader);
    out.timeStamp = ntHeader->FileHeader.TimeDateStamp;

    for (unsigned i = 0; i < ntHeader->FileHeader.NumberOfSections; i++) {
        CHKBUF(mod, section + 1, len, out);
        if (!_stricmp(".text", (char*)section[i].Name)) {
            out.textSize = section[i].SizeOfRawData;
            break;
        }
    }

    return out;
}

ExeInfo GetRemoteExeInfo(void* hProc, uintptr_t mod) {
    ExeInfo out = {};

    SIZE_T byteRet;

    uintptr_t addr_ntHeader;
    ReadProcessMemory(
        hProc,
        (void*)(mod + offsetof(IMAGE_DOS_HEADER, e_lfanew)),
        &addr_ntHeader, sizeof(addr_ntHeader),
        &byteRet
    );
    addr_ntHeader += mod;

    uintptr_t addr_optionalHeader = addr_ntHeader + offsetof(IMAGE_NT_HEADERS, OptionalHeader);
    uintptr_t addr_fileHeader = addr_ntHeader + offsetof(IMAGE_NT_HEADERS, FileHeader);

    ReadProcessMemory(
        hProc,
        (void*)(addr_fileHeader + offsetof(IMAGE_FILE_HEADER, TimeDateStamp)),
        &out.timeStamp, sizeof(out.timeStamp),
        &byteRet
    );

    WORD sizeOfOptionalHeader;
    ReadProcessMemory(
        hProc,
        (void*)(addr_fileHeader + offsetof(IMAGE_FILE_HEADER, SizeOfOptionalHeader)),
        &sizeOfOptionalHeader, sizeof(sizeOfOptionalHeader),
        &byteRet
    );

    WORD numberOfSections;
    ReadProcessMemory(
        hProc,
        (void*)(addr_fileHeader + offsetof(IMAGE_FILE_HEADER, NumberOfSections)),
        &numberOfSections, sizeof(numberOfSections),
        &byteRet
    );

    for (size_t i = 0; i < numberOfSections; i++) {
        IMAGE_SECTION_HEADER section;
        ReadProcessMemory(
            hProc,
            (void*)(addr_optionalHeader + sizeOfOptionalHeader + i * sizeof(IMAGE_SECTION_HEADER)),
            &section, sizeof(section),
            &byteRet
        );

        if (!_stricmp((char*)section.Name, ".text")) {
            out.textSize = section.SizeOfRawData;
            break;
        }
    }
    return out;
}

const THGameVersion* IdentifyExe(const uint8_t* buf, size_t len, ExeInfo* outInfo) {
    auto exe_info = GetExeInfo(buf, len);
    if (outInfo) {
        *outInfo = exe_info;
    }
    if (exe_info) for (const auto& i : gGameVersions) {
        if (exe_info == i.exeInfo) {
            return &i;
        }
    }
    return nullptr;
}

const THGameVersion* IdentifyExe(const wchar_t* path, ExeInfo* outInfo) {
    MappedFile f(path);
    if (!f.fileMapView) {
        return nullptr;
    }
    return IdentifyExe((uint8_t*)f.fileMapView, f.fileSize, outInfo);
}

const THGameVersion* IdentifyRemoteExe(void* hProc, uintptr_t mod, ExeInfo* outInfo) {
    ExeInfo exeInfo = GetRemoteExeInfo(hProc, mod);
    if (!exeInfo) {
        return nullptr;
    }
    if (outInfo) {
        *outInfo = exeInfo;
    }
    for (auto& ver : gGameVersions) {
        if (ver.exeInfo == exeInfo) {
            return &ver;
        }
    }
    return nullptr;
}

void GetExeOepCode(const uint8_t* mod, size_t len, uint16_t (&outOep)[10]) {
    auto* dosHeader = (IMAGE_DOS_HEADER*)mod;
    auto* ntHeader = (IMAGE_NT_HEADERS*)(mod + dosHeader->e_lfanew);

    uintptr_t entry_v = ntHeader->OptionalHeader.AddressOfEntryPoint;
    const uint8_t* entry = nullptr;

    auto* section = (IMAGE_SECTION_HEADER*)((uintptr_t)&ntHeader->OptionalHeader + ntHeader->FileHeader.SizeOfOptionalHeader);
    for (unsigned i = 0; i < ntHeader->FileHeader.NumberOfSections; i++) {
        CHKBUF(mod, section + i + 1, len, );

        if (entry_v >= section[i].VirtualAddress &&
            entry_v <= (section[i].VirtualAddress + section[i].Misc.VirtualSize)) {
            entry = mod + (entry_v - section[i].VirtualAddress + section[i].PointerToRawData);
            break;
        }
    }

    if (entry != nullptr) {
        CHKBUF(mod, entry + sizeof(outOep), len, );
        memcpy(outOep, entry, sizeof(outOep));
        for (size_t i = 0; i < 10; i++) {
            outOep[i] ^= (i + 0x41) | ((i + 0x41) << 8);
        }
    }
}

bool IdentifyKnownGame(THKnownGame& out, uint16_t (&outOepCode)[10], const uint8_t* buf, size_t size, ExeInfo* outInfo) {
    out.ver = IdentifyExe(buf, size, outInfo);
    if (!out.ver) {
        return false;
    }
    MetroHash128::Hash(buf, size, (uint8_t*)out.metroHash);
    GetExeOepCode(buf, size, outOepCode);
    for (const auto& known : gKnownGames) {
        if (known.metroHash[0] == out.metroHash[0] &&
            known.metroHash[1] == out.metroHash[1] &&
            known.metroHash[2] == out.metroHash[2] &&
            known.metroHash[3] == out.metroHash[3]) {
            out.type = known.type;
            return true;
        }
    }

    for (size_t i = 0; i < 10; i++) {
        if (outOepCode[i] != out.ver->oepCode[i]) {
            out.type = TYPE_MALICIOUS;
            return true;
        }
    }

    out.type = TYPE_MODDED;
    return true;
}

bool IdentifyKnownGame(THKnownGame& out, uint16_t (&outOepCode)[10], const wchar_t* fn, ExeInfo* outInfo) {
    MappedFile f(fn, 0x04000000); // 4 MiB
    if (f.fileMapView) {
        return IdentifyKnownGame(out, outOepCode, (const uint8_t*)f.fileMapView, f.fileSize, outInfo);
    } else {
        return false;
    }
}

THGameID ParseExeName(const wchar_t* name, size_t name_len) {
    if (t_str_compare_nocase(name, name_len, COUNTED(L"東方紅魔郷.exe"))) {
        return ID_TH06;
    }

    if (t_str_compare_nocase(name, name_len, COUNTED(L"algostg.exe"))) {
        return ID_ALCOSTG;
    }

    if (!(name_len == 8 || name_len == 9) || ((name[0] | 0x20) != L't') || ((name[1] | 0x20) != L'h')) {
        return ID_UNKNOWN;
    }

    if (name_len == 8) {
        if(name[2] < L'0' || name[2] > L'9' ||
           name[3] < L'0' || name[3] > L'9') {
            return ID_UNKNOWN;
        }
        size_t game_num = (name[2] - L'0') * 10 + (name[3] - L'0');
        switch (game_num) {
            case  6: return ID_TH06;
            case  7: return ID_TH07;
            case  8: return ID_TH08;
            case  9: return ID_TH09;
            case 10: return ID_TH10;
            case 11: return ID_TH11;
            case 12: return ID_TH12;
            case 13: return ID_TH13;
            case 14: return ID_TH14;
            case 15: return ID_TH15;
            case 16: return ID_TH16;
            case 17: return ID_TH17;
            case 18: return ID_TH18;
            case 19: return ID_TH19;
            case 20: return ID_TH20;
            default: return ID_UNKNOWN;
        }
    }
    if (name_len == 9) {
        if(name[2] < L'0' || name[2] > L'9' ||
           name[3] < L'0' || name[3] > L'9' ||
           name[4] < L'0' || name[4] > L'9') {
            return ID_UNKNOWN;
        }
        size_t game_num = (name[4] - L'0') * 100 + (name[3] - L'0') * 10 + (name[4] - L'0');

        switch (game_num) {
            case  75: return ID_TH075;
            case  95: return ID_TH095;
            case 105: return ID_TH105;
            case 123: return ID_TH123;
            case 125: return ID_TH125;
            case 128: return ID_TH128;
            case 135: return ID_TH135;
            case 143: return ID_TH143;
            case 145: return ID_TH145;
            case 155: return ID_TH155;
            case 165: return ID_TH165;
            case 175: return ID_TH175;
            case 185: return ID_TH185;
            default:  return ID_UNKNOWN;
        }
    }

    __assume(0);
}