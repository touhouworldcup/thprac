...
void GameGuiEnd(bool draw_cursor)
{
    ...
    if (draw_cursor && ImGui_ImplWin32_CheckFullScreen()) {
        auto& io = ::ImGui::GetIO();
...
int FPSHelper(adv_opt_ctx& ctx, bool repStatus, bool vpFast, bool vpSlow, bool speedupHeld, bool slowdownHeld, FPSHelperCallback* callback)
{
    ...
    if (ctx.fps_debug_acc && GetAsyncKeyState(VK_SPACE) < 0) {
        if (!isDebugAccActive) {
