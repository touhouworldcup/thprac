...
#define ANM_MANAGER (*(AnmManager** const)0x6d4588)
    ...
    public:
        ...
        {
            switch (mState) {
            case TH06::THPauseMenu::STATE_CLOSE:
                return StateClose();
            case TH06::THPauseMenu::STATE_RESUME:
                return StateResume();
            case TH06::THPauseMenu::STATE_EXIT:
                return StateExit();
            case TH06::THPauseMenu::STATE_RESTART:
                return StateRestart();
            case TH06::THPauseMenu::STATE_OPEN:
                return StateOpen();
            default:
                return SIGNAL_NONE;
            }
    ...
    __declspec(noinline) void THPatch(ECLHelper& ecl, int32_t stage, th_sections_t section)
    {
        ...
        case 6: { // Extra Stage
            ...
            case TH06::TH06_ST7_END_S1:
                ECLNameFix();
                s7b_n1();
                ECLSetTime(ecl, 0x344e, 0x0, 0x0);
                break;

            case TH06::TH06_ST7_END_NS2:
                ECLNameFix();
                s7b_n1();
                s7b_call();
    ...
    EHOOK_ST(th06_sfx_fix, 0x4145c6, 3, {
        ...
        switch (thPracParam.section) {
            case TH06::TH06_ST5_BOSS2:  // It seems that the SFX for this spell varies between SOUND_16 and 
